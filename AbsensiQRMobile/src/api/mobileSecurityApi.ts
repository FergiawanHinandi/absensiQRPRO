...
import { SecurityViolation, SecurityViolationType } from '../services/DeviceSecurityService';
...
class MobileSecurityApi {
    ...
    async reportEvent(
        ...
        const event = this.createSecurityEvent(eventType, severity, deviceFingerprint, details);
        return this.sendEvent(event);
    }
     ...
     */
    private async sendEvent(event: MobileSecurityEvent): Promise<SecurityEventResponse> {
        try {
