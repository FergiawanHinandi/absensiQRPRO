 ...
 * QR Token validation
 */
export const qrToken = (value: string, message = errorMessages.qrToken): ValidationResult => {
  if (isEmpty(value)) return { isValid: true };
 ...
 */
export const deviceId = (value: string, message = errorMessages.deviceId): ValidationResult => {
  if (isEmpty(value)) return { isValid: true };
 ...
 * UUID validation
 */
export const uuid = (value: string, message = 'Format ID tidak valid'): ValidationResult => {
  if (isEmpty(value)) return { isValid: true };
...
export const attendanceScanPayload = (payload: {
  ...
  if (!qrResult.isValid) return qrResult;
