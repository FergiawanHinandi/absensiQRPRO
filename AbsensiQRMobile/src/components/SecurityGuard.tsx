        ...
        <View style={styles.blockedContainer}>
            <View style={styles.blockedCard}>
                ...
                <Text style={styles.blockedMessage}>
                    {primaryViolation?.message || 'Perangkat tidak memenuhi persyaratan keamanan untuk absensi'}
                </Text>
...
export function useDeviceSecurity() {
    const [securityState, setSecurityState] = useState<SecurityCheckResult | null>(null);
    const [isChecking, setIsChecking] = useState(false);

    const checkSecurity = useCallback(async (forceRefresh = false) => {
        setIsChecking(true);
        try {
            const result = await deviceSecurityService.runSecurityChecks(forceRefresh);
