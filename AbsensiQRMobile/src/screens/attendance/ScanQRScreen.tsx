...
import { getValidatedLocation } from '../../utils/locationValidator';
...
const ScanQRContent = ({ navigation }: any) => {
    ...
    const { deviceFingerprint, isSecure, violations, riskLevel } = useDeviceSecurity();
    ...
    const handleScan = async (qrToken: string) => {
        ...
        try {
            // STEP 1: Get RAW location data (no validation)
            ...
            if (response.success) {
                ...
                if (response._idempotent_replay) {
                    Alert.alert(
                        'Sudah Tercatat',
                        `Absensi Anda sudah tercatat sebelumnya pada ${response._original_timestamp}.\n\nStatus: ${attendance.status.toUpperCase()}`,
