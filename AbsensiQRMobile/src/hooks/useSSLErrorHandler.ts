...
export const useSSLErrorHandler = (options: UseSSLErrorHandlerOptions = {}) => {
     ...
     */
    const handleError = useCallback((error: SecureApiError | Error | unknown) => {
     ...
     */
    const isSecurityError = useCallback((error: SecureApiError | Error | unknown): boolean => {
        const secureError = error as SecureApiError;
        return secureError?.type === SecureApiErrorType.SSL_PINNING_FAILED ||
