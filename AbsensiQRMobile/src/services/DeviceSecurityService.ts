...
const VIOLATION_SEVERITY: Record<
...
const VIOLATION_MESSAGES: Record<SecurityViolationType, string> = {
  ...
  jailbroken_device:
    'Perangkat tidak aman untuk absensi (jailbreak terdeteksi)',
  emulator_detected: 'Aplikasi harus dijalankan pada perangkat fisik',
  debug_mode: 'Mode debug tidak diizinkan untuk absensi',
  app_signature_mismatch:
    'Aplikasi termodifikasi terdeteksi. Silakan unduh dari sumber resmi',
...
try {
  ...
  JailMonkey = {
    isJailBroken: async () => !__DEV__,
    isOnExternalStorage: async () => false,
    isDebuggedMode: async () => __DEV__,
    canMockLocation: async () => !__DEV__,
    trustFall: async () => __DEV__,
    isRooted: async () => !__DEV__,
    hookDetected: async () => !__DEV__,
    AdbEnabled: async () => !__DEV__,
    isDevelopmentSettingsMode: async () => !__DEV__,
  };
}
...
class DeviceSecurityService {
  ...
  private readonly PRODUCTION_SIGNATURE_HASH =
    Config.APP_SIGNATURE_HASH || 'YOUR_PRODUCTION_SIGNATURE_SHA256_HASH';
  ...
  private async checkRootStatus(): Promise<boolean> {
    ...
    try {
      ...
      return !__DEV__; // Fail-closed in production
    }
  }
  ...
  private async validateAppSignature(): Promise<boolean> {
    ...
    try {
      ...
      console.warn(
        'AppSignatureModule not available, skipping signature check',
      );
      return true;
    } catch (error) {
      console.warn('Signature validation failed:', error);
      return __DEV__; // Fail-closed in production, open in dev
    }
  }
