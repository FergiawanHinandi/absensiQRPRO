...
export interface QueuedAttendance {
  ...
  /** ISO timestamp when QR was scanned (LOCAL DEVICE TIME) */
  scanned_at?: string;
  /** Client timestamp (epoch ms) for clock skew detection */
 ...
 * Stores LOCAL DEVICE timestamp (scanned_at) to prevent late marking
 * due to network delays during offline sync.
 */
export const addToQueue = async (
  attendance: Omit<QueuedAttendance, 'id' | 'queued_at' | 'retry_count'>,
): Promise<string> => {
  ...
  const queuedItem: QueuedAttendance = {
    ...attendance,
    id: requestId,
    scanned_at: attendance.scanned_at || new Date().toISOString(),
    client_timestamp: attendance.client_timestamp || Date.now(),
...
const syncItem = async (
  ...
  try {
    const payload: ScanPayload = {
      ...
      device_fingerprint: item.device_fingerprint || 'unknown',
      security_context: item.security_context,
      request_id: item.id, // Idempotency key
      // ARCH-03 FIX: Pass LOCAL DEVICE scan timestamp so server uses correct time
      // for attendance status calculation, not the dispatch time.
    ...
    if (status === 401 || status === 403) {
      console.warn(
        `OfflineSync: Auth failed for ${item.id}, keeping for later`,
      );
...
export const syncQueue = async (): Promise<{
  ...
  try {
    ...
    console.log(
      `OfflineSync: Completed - synced: ${synced}, failed: ${failed}, remaining: ${remaining}`
    );
...
class OfflineSyncService {
  ...
  init(): void {
    ...
    // Initialize background fetch sync
    this.initBackgroundSync();
  }

  /**
   * Initialize background sync using react-native-background-fetch
   */
  async initBackgroundSync(): Promise<void> {
    ...
    console.log('✅ OfflineSync: Background sync configured');
  }
