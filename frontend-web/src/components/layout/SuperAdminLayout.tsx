...
import { Menu, X, ChevronDown, Bell, LogOut, User, Clock, Settings } from "lucide-react";
...
export const SuperAdminLayout: React.FC = () => {
  ...
  // Menu structure - using centralized navigation config
  const menuItems = MENUS.super_admin.map((item) => ({
    ...item,
    id: item.label.toLowerCase().replace(/\s+/g, "-").replace(/[&]/g, "dan"),
  }));
  ...
  return (
    <div className="flex h-screen bg-gradient-to-br from-gray-900 via-gray-900 to-black text-gray-100 overflow-hidden">
      ...
      {/* Sidebar */}
      <aside
        className={`fixed lg:static inset-y-0 left-0 z-50 w-72 bg-gray-900 border-r border-gray-800 flex flex-col transform transition-transform duration-300 lg:transform-none ${
        ...
        <div className="h-16 bg-gray-900 border-b border-gray-800 px-6 flex items-center justify-between sticky top-0 z-30 shadow-lg">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative" ref={notificationRef}>
              ...
              >
                <Bell
                  className={`w-5 h-5 ${notificationDropdownOpen ? "text-white" : "text-gray-400 group-hover:text-white"}`}
                />
                {hasUnread && (
                  ...
                  <div className="max-h-[320px] overflow-y-auto">
                    {notifications.length > 0 ? (
                      notifications.map((notif: any, idx: number) => (
                        <div
                          key={idx}
                          className="px-4 py-3 border-b border-gray-700/50 hover:bg-gray-700/50 transition-colors cursor-pointer group"
                        >
                          <div className="flex items-start gap-3">
                            <div
                              className={`mt-0.5 min-w-[8px] h-2 rounded-full ${
                                notif.type === "error"
                                  ? "bg-red-500"
                                  : notif.type === "warning"
                                    ? "bg-amber-500"
                            ...
                            <div className="flex-1 min-w-0">
                              <p className="text-sm font-medium text-gray-200 truncate mb-0.5 group-hover:text-blue-400 transition-colors">
                                {notif.title}
                              </p>
