 ...
 *
 * NOTE: apiClient interceptor di lib/api.ts sudah auto-unwrap response.data.data → response.data
 *       sehingga tidak perlu .data.data lagi di sini.
 */
...
export const getDailyReport = async (date?: string) => {
    ...
    // FE-01 FIX: interceptor sudah unwrap, pakai .data langsung
...
// FE-02 FIX: Perbaiki path dari /export-excel → /export/excel sesuai route backend
